...
if selected=='研究内容':
    st.title('⚕️研究内容')
    st.header('一、研究背景')
    '胃癌术后低体重是术后并发症及长期预后的重要危险因素，研究进一步分析发现在体重下降过程中，瘦肉组织群的丢失是重要的预后因素，因此阻击胃癌术后瘦肉组织群丢失是改善胃癌患者预后的重要治疗措施。我们基于自主开发的机体组成自动化检测软件，通过患者CT图像多时空的追踪患者机体机组成改变，其中包含骨骼肌群、内脏、皮下脂肪群，通过定量分析，指导术后营养支持策略调整。本研究中我们给予胃癌患者术后长期***进行补充能量，降低患者术后摄入不足对全身营养状态的影响，同时供给益生菌改善优化肠道菌群，通过肠道菌群及其代谢产物与骨骼肌群间的互相对话，改善骨骼肌质量（前期研究已证实优化肠道菌群可以改善骨骼肌质量）。基于上述的科研思路，我们设计如下临床随机对照研究，进一步从临床层面上观察***是否可以改善术后骨骼肌丢失。'
    '--------'         

    st.header('二、研究目的')
    '1. 主要目的：基于机体组成变化探究长程****干预对****行全胃切除术后骨骼肌流失的影响。'
    '2. 次要目的：探究长程****干预对*****行全胃切除术后骨骼肌流失影响的可能作用机制及患者的生存预后和营养状况。'
    st.header('三、研究设计类型、原则与试验步骤')
    '1. 研究设计'
    '本研究是多中心、平行、开放、优效的随机对照临床研究，预计5家医院参与本研究。针对主要终点指标，采用log-rank检验，按照双侧显著性水平α和检验效能1-β，采用PASS15.0软件对本试验所需的样本量进行估算，并且考虑5%左右的脱落率，最终预计纳入696例老年Ⅲ期胃癌行全胃切除术的患者。每个中心采用竞争入组的方式纳入患者，采用分层区组随机化的方法将纳入的患者按1：1：1随机分为****、***和常规饮食组，每组为232人。'
    '2. 研究步骤'
    ...
    '治疗方案：'
    'ONS联合益生菌组: 随机后口服肠内营养补充剂安素，（150g/d）+****3个月'
    ...
    ***和***组
    ***：***，每日按照最低目标量喂养：热卡25-30kcal/kg，蛋白质：1.0-1.5g/kg，一共干预3个月。
    ***：
...
'''
    ...
    '''
1、理论创新程度
研究视角的创新：该课题将机体组成变化、***干预三个元素相结合，针对老年III期胃癌全胃切除术后患者这一特定人群，探讨其对骨骼肌流失的影响。这种跨学科的整合视角在胃癌术后康复领域是较为新颖的。
研究方法的创新：通过监测机体组成变化，尤其是骨骼肌流失的情况，能够更直观地评估长程ONS联合益生菌干预的效果。这种定量评估方法相较于传统的临床评估更为精确和客观。
理论模型的创新：课题可能提出或验证一个新的理论模型，即***干预能够改善老年胃癌术后患者的营养状态，进而减少骨骼肌流失，促进康复。
2、实际应用价值
    ...
    with st.container(height=800,border=True):
        with pages:
            if pages.current == 0:#第一页，患者基本信息
                ...
                survey.selectbox("采取何种治疗措施？",options=["常规饮食组", "***",'￥￥￥￥'],id="treatment")
